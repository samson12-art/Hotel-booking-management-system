...
import {
  ...
  confirmBooking as svcConfirmBooking,
  rejectBooking as svcRejectBooking,
  checkInGuest as svcCheckInGuest,
  checkOutGuest as svcCheckOutGuest,
} from "../services/booking.service";

export const createBooking = async (req: AuthRequest, res: Response) => {
  try {
    const data = bookingSchema.parse(req.body);
    const result = await svcCreateBooking({ ...data, userId: req.user!.id });
...
export const cancelBooking = async (req: AuthRequest, res: Response) => {
  try {
    const result = await svcCancelBooking(req.params.id, req.user!.id, req.user!.role);
    sendSuccess(res, "Booking cancelled", result);
  } catch (error: any) {
    const status = error.message === "Booking not found." ? 404
      : error.message === "Not authorized." ? 403 : 400;
...
export const confirmBooking = async (req: AuthRequest, res: Response) => {
  try {
    const updated = await svcConfirmBooking(req.params.id);
...
export const rejectBooking = async (req: AuthRequest, res: Response) => {
  try {
    const updated = await svcRejectBooking(req.params.id);
...
export const checkInGuest = async (req: AuthRequest, res: Response) => {
  try {
    const updated = await svcCheckInGuest(req.params.id);
...
export const checkOutGuest = async (req: AuthRequest, res: Response) => {
  try {
    const updated = await svcCheckOutGuest(req.params.id);
